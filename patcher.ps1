...
$patterns = @{
    MachineId = @{
        ...
        Probe   = '=\/\*csp1\*\/\"[\s\S]*?\"\/\*1csp\*\/,';
        ProbeReplace = "=/*csp1*/`"$machineId`"/*1csp*/,";
    };
    MacAddress = @{
...
foreach ($key in $patterns.Keys) {
    ...
    elseif ($content -match $pattern.Probe) {
        $content = $content -replace $pattern.Probe, $pattern.ProbeReplace
